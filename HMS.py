...
def Ref():
    ...
    costofmeal = (costoffries +  costoflargefries + costofburger + costoffilet + costofcheeseburger + costofdrinks)
    ...
    cur.execute(csql)
    sql=f"insert into bills values('{randomRef}',{cof},{colfries},{cob},{cofi},{cochee},{codr},'{Service}','{costofmeal}','{PaidTax}','{costofmeal}','{OverAllCost}')"
    print(sql)
    cur.execute(sql)
    conn.commit()
    cur.close()
...
status = Label(f2,font=('aria', 15, 'bold'),width = 16, text="-By Kishore Kumar",bd=2,relief=SUNKEN)
